...
public class AUTImporter {
	...
	public static CompactDFA<Character> read(@WillClose InputStream is) throws IOException {
		try(Scanner sc = new Scanner(uncompressed(is))) {
			...
			Alphabet<Character> alphabet = Alphabets.characters('a', (char) (((int) 'a') + numSymbols - 1));
			
			CompactDFA<Character> result = new CompactDFA<>(alphabet, numStates);
			...
			for(int i = 0; i < numStates; i++) {
				...
				for(int j = 0; j < numSymbols; j++) {
					int succ = states[sc.nextInt()];
                                        char a = (char) (((int) 'a') + j);
                                        //System.out.println("" + state + " : " + a + " : " + succ);
